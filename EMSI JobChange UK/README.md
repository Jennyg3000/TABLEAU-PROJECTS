...
1.  **`retail_sales_dataset_Master.xlsx`**: Contains raw transaction data and smaller datasets used for hands-on Excel exercises (Pivot Tables, Functions, etc.). ✍️
...
3.  **`Employment Dataset.twbx`**: The final interactive Tableau/Power BI dashboard visualizing the UK Job Change data. 🗺️
...
| :--- | :--- | :--- | :--- |
| `retail_sales_dataset.csv` | **Raw Transactional Data** | `Date`, `Product Category`, `Gender`, `Generation`, `Total Sales` | The core dataset for trend analysis and detailed customer/product segmentation. 🛍️ |
| `retail_sales_dataset_Master.xlsx - Sheet1.csv` | **Pivot Table (Proportional)** | `Product Category`, `Gender`, `Generation` (Values are **percentages** of total sales) | Shows the distribution of market share across segments for each product. 🥧 |
| `retail_sales_dataset_Master.xlsx - Sheet2.csv` | **Pivot Table (Absolute)** | `Product Category`, `Gender`, `Adult` (Values are **absolute sales** figures) | Summarizes absolute sales figures, typically filtered or focused on a specific segment. 💵 |
| `retail_sales_dataset_Master.xlsx - Day 2 Task 2.csv` | **Student Scores** | `Student name`, `English`, `Mathematics`, `Science`, `Average` | Data used for Excel functional tasks like calculating `AVERAGE` and `MAX` values. 🧠 |
...
| **`Sheet2.csv`** (Absolute) | **Clustered Bar Chart** | Compares **Absolute Sales** by `Product Category` across different dimensions to identify key revenue drivers. 💰 |
...
The core of the UK Job Change project is the interactive dashboard, built on the `EMSI_JobChange_UK.xlsx` data.

1.  **Geographic Map of Job Change**: 🗺️
